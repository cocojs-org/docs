  ...
  code1: string = `
{
  ...
  render() {
    return <ContentLayout sideMenu={<SideMenu />}>
      ...
      <Code code={this.code} />
      最外层的key表示组件的id，对应的value表示做的配置，这里做了2个事：
      1. bootComponents：用于统一配置需要自启动的组件，值是一个对象，对象的key是组件的id，value是truthy的都会自启动
      ...
      <Card>
        1. coco dev默认启用application.dev.json，不需要通过NODE_ENV显式设置，当然也可以通过NODE_ENV去加载其他的配置文件
        2. coco build默认启用application.prod.json，不需要通过NODE_ENV显式设置，当然也可以通过NODE_ENV去加载其他的配置文件
      </Card>
      <Header2>配置合并逻辑</Header2>
      json的值类型有：string number boolean null Object Array
      我们假设执行coco build命令，也就是application.json和application.prod.json合并，
      application.json简称o1，application.prod.json简称o2，合并得到o3，合并逻辑如下：
      1. 遍历o2的所有key，比较o1[key]和o2[key]：
       1.1. o1没有[key]，取o2的值，o3[key] = o2[key]
       1.2. o1有[key]：
         1.2.1 o1[key] o2[key] 类型不一致，则取o2的值，o3[key] = o2[key]
         1.2.2 o1[key] o2[key] 类型一致：
           1.2.2.1 如果类型是string number boolean null array，则取o2的值，o3[key] = o2[key]
           1.2.2.2 如果类型是Object，则走第1步
