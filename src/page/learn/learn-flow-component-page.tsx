  ...
  render() {
    return <ContentLayout sideMenu={<SideMenu />}>
      ...
      <div>
        ...
        我们可以将这些外部 API 简单的分为 2 类：UI副作用和数据副作用。
        1. UI副作用：是指包含和用户交互相关的副作用，例如
        <ol>
          <li>input.focus()</li>
        ...
        <ol>
          <li>setTimeout(() => ..., 1000)</li>
          <li>setInterval(() => ..., 1000)</li>
        ...
        总结一下拆分出数据副作用的好处：
        1. 只关心业务的数据流转过程
        2. 可以被任意页面调用，不用担心页面状态被修改
        3. 业务逻辑高聚合
        4. UI 副作用适合封装视图层的业务逻辑，而数据副作用适合封装数据处理的流程
        所以框架提供了<InlineCode>@flow</InlineCode>装饰器封装数据副作用，或者说封装数据的处理流程，这也是取名 flow 的原因。
        <Card>
          我们认为UI副作用和数据副作用的影响范围是不同的，使用场景也是不同的：UI 副作用适合封装视图层的业务逻辑，而数据副作用适合封装数据处理的流程。
      ...
      <Card>
        ...
        我们推荐是不需要，因为这样的封装是没有意义的，是为了封装而封装，视图组件同样可以直接调用后端接口组件，当业务变复杂，需要调用多个副作用了，那么再提取成 flow 组件也不迟。
      </Card>
    </ContentLayout>
  }
}
