    ...
    render() {
        return (
            <ContentLayout sideMenu={<SideMenu />}>
                ...
                他们通过createDecoratorExp绑定，假设一个名为Button的类使用了@view装饰器，那么运行时框架会为Button关联一个View实例，这样框架就知道
                Button 是一个视图组件。
                <Code code={this.buttonClass} />
                <Card>
                    一般来说，元数据类名和装饰器名字应该保持一致（除了首字母大小写不同）。例如
                    <InlineCode>View</InlineCode>和<InlineCode>view</InlineCode>
                </Card>
                <Card>
                ...
                1. 如果类型是纯对象（即<InlineCode>{}</InlineCode>
                ），则找到该对象自身全部可枚举的field，浅赋值给元数据实例 2. 其他类型： 2.1
                如果元数据类定义了field，那么会获取第一个field，进行赋值 2.2
                如果元数据类没有定义field，那么默认赋值给"value"这个field
                <Card>
                    1. 只会使用装饰器表达式的第一个参数 2.
                    如果希望接受多个装饰器参数，只能为元数据类定义多个field，同时装饰器参数只能使用纯对象 3.
                    如果元数据类只有一个field，且不是赋值给默认值，那么需要给key添加默认值，哪怕是undefined
                </Card>
            </ContentLayout>
        );
    }
